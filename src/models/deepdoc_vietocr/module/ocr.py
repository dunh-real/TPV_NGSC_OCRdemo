...
def load_model(model_dir, nm, device_id: int | None = None):
    ...
    def cuda_is_available():
        try:
            import torch
            if torch.cuda.is_available() and device_id is not None and torch.cuda.device_count() > device_id:
                return True
            if torch.cuda.is_available() and device_id is None:
                return True
        except Exception:
            return False
        return False
    ...
    if cuda_is_available():
        ...
        if actual_provider == 'CUDAExecutionProvider':
            run_options.add_run_config_entry("memory.enable_memory_arena_shrinkage", "gpu:" + str(device_id if device_id is not None else 0))
            logging.info(f"load_model {model_file_path} uses GPU")
        else:
            run_options.add_run_config_entry("memory.enable_memory_arena_shrinkage", "cpu")
            logging.warning(f"load_model {model_file_path} requested GPU but fell back to {actual_provider}")
    else:
        sess = ort.InferenceSession(
            model_file_path,
            options=options,
...
class TextRecognizer:
    def __init__(self, model_dir=None, device_id: int | None = None):
        ...
        logging.info(f"VietOCR TextRecognizer uses device: {config['device']}")
